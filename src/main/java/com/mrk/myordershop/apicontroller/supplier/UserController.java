...
@RequestMapping(value = "/api/v1/sp/users")
...
public class UserController {
	...
	public ResponseEntity<PagedResources<UserSearchResource>> searchOnOrder(
			@Owner Supplier user, @ModelAttribute UserSearchFilter filter,
	...
	public ResponseEntity findUser(@ModelAttribute UserFilter filter,
		...
		if (filter != null)
			filter.setRole(Role.ROLE_WSALER);
		Page<User> users = userService.find(filter, pageable, supplier);
