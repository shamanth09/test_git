...
var orderApp = angular.module("app.orders", [ "app", 'ngRoute', "ui.bootstrap",'ngSanitize', 'ui.select']).config([ "$provide", function($provide) {
	...
	$routeProvider.otherwise("/").when("/", {
		templateUrl : function(params) {
			...
			return UrlProvider.buildUrl("orders/xhr/list", params);
		},
	...
	$routeProvider.when("/orders/:orderId", {
		templateUrl : function(params) {
			params.activeId = activeId;
			return UrlProvider.buildUrl("orders/xhr/:orderId", params);
		},
	...
	$routeProvider.when("/orders/:orderId/edit", {
		templateUrl : function(params) {
			params.activeId = activeId;
			return UrlProvider.buildUrl("orders/xhr/:orderId/edit", params);
		},
	...
	$scope.searchInOrder = function(q){
		if(q.length >= 1){
			$http.get('search?q='+q+'&fields=customerMobile&fields=customerName&fields=userName&fields=productName').then(function(resp){
	...
	for(var i in $routeParams){
		if(i=='customerName'||i=='customerMobile' || i=='userName' || i=='productName' ){
			if($routeParams[i] instanceof Array){
				var values = $routeParams[i];
				for(var ind in values){
					$scope.orderFilter.push({'field':i,'result':values[ind],'label':{'customerName':"in customers",'customerMobile':"in customers",'productName':" in Product Name",'userName':"in User Name"}[i]})
				}
			}else {
				$scope.orderFilter.push({'field':i,'result':$routeParams[i],'label':{'customerName':"in customers",'customerMobile':"in customers",'productName':" in Product Name",'userName':"in User Name"}[i]})
			}
		}
	}
	...
	$scope.onClickUser = function(orderId) {
		$location.path("/orders/" + orderId).search({});
	};
	...
	{
		if(list.length > 0){
			$scope.measurments=list.map(function(o){
				console.log("--------ms"+o.measurement);
	...
	$scope.updateOrder = function(formData, actionUrl) {
		$http.post(actionUrl, formData).then(function(resp) {
			$location.path("/orders/" + resp.data.id).search({})
		}, function(error) {
		});
	}
